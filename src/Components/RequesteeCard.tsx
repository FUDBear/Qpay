...
const RequesteeCard: React.FC<Requestee> = ({ Address, Amount, UpdateRequestee, RemoveRequestee }) => {
  const showDeleteConfirm = () => {
    if (window.confirm("Are you sure you want to delete this requestee?")) {
      RemoveRequestee();
    }
  };
    ...
    <div className="bg-slate-50 shadow-lg rounded-lg p-6 w-full relative">
      ...
      <div>
        ...
        <input
          type="text"
          value={Amount}
          onChange={(e) => UpdateRequestee("Amount", e.target.value)}
          className="border bg-slate-100 rounded-lg py-2 px-4 w-full focus:outline-none focus:border-[#4318FF] focus:ring-1 focus:ring-[#4318FF]"
